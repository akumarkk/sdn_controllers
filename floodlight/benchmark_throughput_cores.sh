...
test_controller()
{
	...
	for THR_NUM in $(seq 1 $THR_NUM) ; do
		...
		echo "^^^^^^^^^^^^^^^^^^^^^^^^  START TEST (Threads: $THR_NUM) ^^^^^^^^^^^^^^^^^^^^^^^^^" 
...
for i in $(seq 0 1 $CONTR_NUM) ; do
	echo "switches: $FIX_SWITCH" >> $log;
	echo "**********   Starting test (SWITCHES = $FIX_SWITCH | HOSTS = $FIX_MAC)  ***************"
	test_controller $i $FIX_MAC $FIX_SWITCH "-t" $THR_NUM;
done
